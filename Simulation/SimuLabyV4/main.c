...
int bonneBalle = 1; // Bonne ou mauvaise balle
int sonar = 0; // Balle proche du robot (1) ou non (0)
...
// Variables x et y pour le balayage de la carte
int x = 0;
int y = 0;

// Variables de temps pour la fonction dessiner_map()
int temp_A = 0;
int temp_B = 0;
int cpt = 0;
						...
						while(sortieballe)
						{
							for(i = 0; i < 9; i++)
							{
								if ((choix==2 || choix==3) && i==3)
									i++; // On passe à la balle suivante
								...
								if(sonar)
								{
									// bonneBalle = testBalle() Teste la couleur de la balle
								...
								if(i == 8) // Si le robot est allé à tous les emplacements des balles, alors on va vers la sortie
								{
									sortieballe = 0;
								}
							}
...
// Effacer l'ancien chemin afin d'en déterminer un autre
void unpropagate(int robot_x, int robot_y)
	{
